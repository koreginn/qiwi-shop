...
class OrdersController extends VoyagerBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $newStatusOrder = $request->status;

        switch ($request->status) {
            case "open":
                $newStatusOrder = "Заказ сделан на сайте, ожидает подтверждение менеджера";
                break;
            case "confirmed":
                $newStatusOrder = "Заказ подтвержден, готовится к доставке";
                break;
            case "sent":
                $newStatusOrder = "Заказ отправлен";
                break;
            case "delivered":
                $newStatusOrder = "Заказ доставлен";
                break;
            case "canceled":
                $newStatusOrder = "Заказ отменен";
                break;
        }
        ...
        Mail::send(['text' => 'status'], ['data' => $data], function($message) use ($data) {
            $message->to($data['email'], 'Изменение статуса заказа')->subject('Изменение статуса заказа');
            $message->from('feedback@mywonders.ru', 'Служба опевещений');
        });
